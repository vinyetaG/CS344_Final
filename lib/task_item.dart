...
///Name: Gerald Vinyeta
///Date: 12/14/2022
///Description: A list tile containing all relevant information for its
///             associated task.
///Bugs: N/A
///Reflection: The timer was the most challenging and time consuming part of the
///            project, the rest is relatively straightforward.
...
class TaskItemState extends State<TaskItem> {
    ...
    if (!widget.timerRunning) {
      ...
      _secondsToTimer(
          (DateTime.now().millisecondsSinceEpoch - widget.timerStarted!) ~/
              1000);
      //Update seconds elapsed to account for time elapsed since periodic timer
      //was canceled
  ...
  void _toggleTimer() {
    if (widget.timerRunning) {
      ...
      timerIncrement = Timer.periodic(
          const Duration(seconds: 1), (timer) => _incrementTimer());
      //Update new timer start time to account for the time that has passed
      //since it was paused
      widget.timerStarted =
  ...
  //Returns the appropriate play/pause button depending on timer state
  Icon get timerControlIcon {
    if (widget.timerRunning) {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      subtitle: Text(widget.description ?? '', overflow: TextOverflow.ellipsis),
